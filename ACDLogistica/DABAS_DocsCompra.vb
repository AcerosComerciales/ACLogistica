...
#Region " Procedimientos Almacenados "
   ...
   Public Function LOG_DOCCOSS_TodosDocCompra(ByVal m_listabas_docscompra As List(Of EABAS_DocsCompra), ByVal x_zonas_codigo As String, ByVal x_sucur_id As Short, ByVal x_cadena As String, ByVal x_opcion As Short, ByVal x_todos As Boolean, ByVal x_fecini As Date, ByVal x_fecfin As Date) As Boolean
    ...
    ''' <summary> 
    ''' Capa de Datos: LOG_ABASS_BusDocsCompra
    ''' </summary>
    ...
    ''' <returns></returns> 
    ''' <remarks></remarks> 
    Public Function LOG_ABASS_BusDocsCompra(ByVal m_listabas_docscompra As List(Of EABAS_DocsCompra), ByVal x_fecini As Date, ByVal x_fecfin As Date, ByVal x_almac_id As Long, ByVal x_cadena As String, ByVal x_todos As Boolean) As Boolean
        Try
            DAEnterprise.AsignarProcedure("LOG_ABASS_BusDocsCompra")
            DAEnterprise.AgregarParametro("@FecIni", x_fecini, DbType.DateTime, 8)
            DAEnterprise.AgregarParametro("@FecFin", x_fecfin, DbType.DateTime, 8)
   ...
   Public Function COSTESS_ObtenerDocCosteados(ByRef m_abas_docscompra As EABAS_DocsCompra, _
    ...
    ''' <summary> 
    ''' Capa de Datos: ABAS_DOCCOSS_DocumentosXNotaCredito
    ''' </summary>
    ''' <param name="x_docco_codigo">Parametro 1: </param> 
    ''' ''' <param name="x_entid_codigo">Parametro 2: </param> 
    ''' <returns></returns> 
    ''' <remarks></remarks> 
    Public Function ABAS_DOCCOSS_DocumentosRelacionados(ByVal m_listnotas_credito As List(Of EABAS_DocsCompra), ByVal x_docco_codigo As String, ByVal x_entid_codigo As String) As Boolean
        Try
            DAEnterprise.AsignarProcedure("ABAS_DOCCOSS_DocumentosRelacionados")
            ...
            Using reader As DbDataReader = DAEnterprise.ExecuteDataReader()
                If reader.HasRows Then
                    While reader.Read()
                        ...
                        ACEsquemas.ACCargarEsquema(reader, _abas_docscompra)
                        _abas_docscompra.Instanciar(ACEInstancia.Consulta)
                        m_listnotas_credito.Add(_abas_docscompra)
                    End While
                    Return True
                Else
                    Return False
