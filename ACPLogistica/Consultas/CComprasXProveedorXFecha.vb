...
Public Class CComprasXProveedorXFecha
#Region " Variables "
...
#Region " Metodos "
   ...
   Private Sub formatearGrilla()
      ...
      Try
         ...
         ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdBusqueda, index, "ARTIC_Codigo", "ARTIC_Codigo", "ARTIC_Codigo", 150, True, False, "System.String") : index += 1
   ...
   Private Sub Cargar()
      Dim index As Integer = 1
      Try
         Dim _cols As Integer = m_reporte.Tables(0).Rows.Count
         Dim _rows As Integer = m_reporte.Tables(1).Rows.Count
         ACFrameworkC1.ACUtilitarios.ACFormatearGrilla(c1grdBusqueda, 2, 2, (_cols + 3), 1, 0)
         ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdBusqueda, index, "Codigo", "ARTIC_Codigo", "ARTIC_Codigo", 150, True, False, "System.String") : index += 1
         ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdBusqueda, index, "Articulo", "ARTIC_Descripcion", "ARTIC_Descripcion", 150, True, False, "System.String") : index += 1
         '' Cargar Cabecera
         Dim rg As CellRange
         Dim _ini As Integer = 3 : Dim _fin As Integer = 3 : Dim _top As Boolean = True
         Dim _anho As Integer = m_reporte.Tables(0).Rows(0)("Anho")
         Dim _anhoFin As Integer
         Dim j As Integer = 3
         For Each Item As DataRow In m_reporte.Tables(0).Rows
            Dim _mes As String = String.Format("{0}_{1}", Item("Anho"), Item("Mes"))
            ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdBusqueda, index, MonthName(Item("Mes")), _mes, _mes, 150, True, False, "System.String") : index += 1
            c1grdBusqueda.Rows(1)(j) = c1grdBusqueda.Rows(0)(j)
            If Item("Anho") <> _anho Then
               rg = c1grdBusqueda.GetCellRange(0, _ini, 0, _fin - 1)
               _ini = _fin
               rg.Data = _anho
               _anho = Item("Anho")
               c1grdBusqueda.MergedRanges.Add(rg)
         ...
         For k As Integer = 0 To 2
            rg = c1grdBusqueda.GetCellRange(0, k, 1, k)
            c1grdBusqueda.MergedRanges.Add(rg)
         Next
         '' Cargar Lineas
         c1grdBusqueda.Rows.Count += _rows
         Dim i As Integer = 2
         For Each Item As DataRow In m_reporte.Tables(1).Rows
            c1grdBusqueda.Rows(i)("ARTIC_Codigo") = Item("ARTIC_Codigo")
            c1grdBusqueda.Rows(i)("ARTIC_Descripcion") = Item("ARTIC_Descripcion")
            i += 1
         Next
         '' Cargar Datos
         For k As Integer = 2 To c1grdBusqueda.Rows.Count - 1
            Dim _select As String = String.Format("ARTIC_Codigo = {0}", c1grdBusqueda.Rows(k)("ARTIC_Codigo"))
            For Each Data As DataRow In m_reporte.Tables(2).Select(_select)
               Dim _fecha As DateTime = Data("DOCCO_FechaDocumento")
               Dim _mes As String = String.Format("{0}_{1}", _fecha.Year, _fecha.Month)
               c1grdBusqueda.Rows(k)(_mes) += Data("DOCCD_Cantidad")
            Next
         Next
         c1grdBusqueda.AutoSizeCols()
      Catch ex As Exception
...
#Region " Metodos de Controles"
   ...
   Private Sub btnCargar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCargar.Click
      Try
         ...
         If Not managerGenerarReportes.PendientesXProveedorXFecha(x_entid_codigo, AcFecha.ACDtpFecha_De.Value.Date, AcFecha.ACDtpFecha_A.Value.Date.AddDays(1), m_reporte) Then
