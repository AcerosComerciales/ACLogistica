...
#Region " Constructores "
   Public Sub New(ByVal x_opcion As ETipos.TipoDocPago, ByVal x_busqueda As Boolean, ByVal x_inicio As TipoInicio, ByVal x_puntoventa As Long)
      ...
      Try
         ...
         managerBDocsPago = New BTESO_DocsPagos(m_puntoventa, GApp.Periodo, GApp.Zona, GApp.Sucursal)
      ...
      Try
         If Not IsNothing(bs_cuentas.Current) Then
            Select Case m_opcion
               ...
               Case ETipos.TipoDocPago.Cheque
                  cmbChTipoCuenta.SelectedValue = CType(bs_cuentas.Current, ECuentas).TIPOS_CodTipoCuenta
                  cmbChMoneda.SelectedValue = CType(bs_cuentas.Current, ECuentas).TIPOS_CodTipoMoneda
               Case Else
                  cmbDeTipoCuenta.SelectedValue = CType(bs_cuentas.Current, ECuentas).TIPOS_CodTipoCuenta
                  cmbDeMoneda.SelectedValue = CType(bs_cuentas.Current, ECuentas).TIPOS_CodTipoMoneda
            End Select
...
#Region " Utilitarios "
   Private Sub setTipoDocumento()
      ...
      Select Case m_opcion
         Case ETipos.TipoDocPago.Cheque
            ACUtilitarios.ACLimpiaVar(Panel1)
            Me.Size = New Size(Me.Width, 425)
            tabMantenimiento.SelectedTab = tabCheque
            acpnlTitulo.ACCaption = String.Format(acpnlTitulo.ACCaption, "Cheques")
            Me.Icon = Icon.FromHandle(ACPLogistica.My.Resources.cheque_16x16.GetHicon)
            bs_bancos_CurrentChanged(Nothing, Nothing)
         Case ETipos.TipoDocPago.Deposito
            ACUtilitarios.ACLimpiaVar(tabDeposito)
            ...
            Me.Icon = Icon.FromHandle(ACPLogistica.My.Resources.deposito_16x16.GetHicon)
            bs_bancos_CurrentChanged(Nothing, Nothing)
         Case ETipos.TipoDocPago.NotaCredito
            acpnlTitulo.ACCaption = String.Format(acpnlTitulo.ACCaption, "Nota de Crédito")
            ACUtilitarios.ACLimpiaVar(tpgNotaCredito)
            Me.Size = New Size(500, 355)
            cmbNCTipoDocumento.Enabled = False
            tabMantenimiento.SelectedTab = tpgNotaCredito
            Me.Icon = Icon.FromHandle(ACPLogistica.My.Resources.Documento_16x16.GetHicon)
         ...
         Case ETipos.TipoDocPago.Letra
            ...
            Me.Icon = Icon.FromHandle(ACPLogistica.My.Resources.letra_16x16.GetHicon)
         Case ETipos.TipoDocPago.Detraccion
            ACUtilitarios.ACLimpiaVar(tpgDetraccion)
         ...
         Case ETipos.TipoDocPago.RecEgreInterno
            ACUtilitarios.ACLimpiaVar(tpgRecEgreInterno)
      ...
      Try
         Select Case m_opcion
            ...
            Case ETipos.TipoDocPago.NotaCredito
               ...
               ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdBusqueda, index, "Codigo", "DPAGO_Id", "DPAGO_Id", 150, True, False, "System.String", "00000") : index += 1
               ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdBusqueda, index, "Fecha", "DPAGO_Fecha", "DPAGO_Fecha", 150, True, False, "System.DateTime", Parametros.GetParametro(EParametros.TipoParametros.pg_FormatoFecha)) : index += 1
               ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdBusqueda, index, "Nro. Doc.", "DocVenta", "DocVenta", 150, True, False, "System.String", "") : index += 1
               ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdBusqueda, index, "R.U.C.", "ENTID_Codigo", "ENTID_Codigo", 150, True, False, "System.String") : index += 1
   ...
   Private Sub cargarCombos()
      Try
         ...
         Colecciones.GetBancos()
         Colecciones.GetNroCuentas()
         ...
         ACUtilitarios.ACCargaCombo(cmbDeMoneda, Colecciones.Tipos(ETipos.MyTipos.TipoMoneda), "TIPOS_DescCorta", "TIPOS_Codigo")

         ACUtilitarios.ACCargaCombo(cmbChTipoCuenta, Colecciones.Tipos(ETipos.MyTipos.TipoCuentaBancaria), "TIPOS_Descripcion", "TIPOS_Codigo")
         ACUtilitarios.ACCargaCombo(cmbChMoneda, Colecciones.Tipos(ETipos.MyTipos.TipoMoneda), "TIPOS_DescCorta", "TIPOS_Codigo")

         ACUtilitarios.ACCargaCombo(cmbDeModoPago, Colecciones.Tipos(ETipos.MyTipos.ModoPago), "TIPOS_Descripcion", "TIPOS_Codigo")
      ...
      Try
         Select Case m_opcion
            ...
            Case ETipos.TipoDocPago.Cheque
               ...
               m_listBindHelper.Add(ACBindHelper.ACBind(cmbChMoneda, "SelectedValue", m_edocpago, "TIPOS_CodTipoMoneda"))

               If m_edocpago.Nuevo Then cmbChBanco.SelectedIndex = 0
               ''Deposito
            Case ETipos.TipoDocPago.Deposito
               ...
               m_listBindHelper.Add(ACBindHelper.ACBind(cmbDeModoPago, "SelectedValue", m_edocpago, "TIPOS_CodModoPago"))
   ...
   Private Sub cargar()
      Try
         If bs_docsPago.Current IsNot Nothing Then
            Dim x_pvent As Integer = CType(bs_docsPago.Current, ETESO_DocsPagos).PVENT_Id
            Dim x_codigo As Integer = CType(bs_docsPago.Current, ETESO_DocsPagos).DPAGO_Id

            managerBDocsPago.Cargar(x_pvent, x_codigo)
...
#End Region
   ...
   Private Function busqueda(ByVal x_cadena As String, ByVal x_opcion As Short) As Boolean
      Try
         If managerBDocsPago.Busqueda(x_cadena, ETipos.getTipoDocPago(m_opcion), x_opcion, acFecha.ACDtpFecha_De.Value.Date, acFecha.ACDtpFecha_A.Value.Date.AddDays(1), chkTodos.Checked) Then '
   ...
   Private Sub NuevoCliente(ByVal x_entid_nrodocumento As String)
      Try
         ...
         If frmEntidad.ShowDialog() = System.Windows.Forms.DialogResult.OK Then
            ...
            m_edocpago.VENT_DocsVenta.ENTID_CodigoCliente = frmEntidad.EEntidad.ENTID_Codigo
            ...
            If Ayuda.ShowDialog() = System.Windows.Forms.DialogResult.OK Then
               Select Case x_opcion
                  Case EEntidades.TipoEntidad.Clientes
                     ...
                     m_edocpago.VENT_DocsVenta.ENTID_CodigoCliente = Ayuda.Respuesta.Rows(0)("Codigo")
                     m_edocpago.ENTID_Codigo = m_edocpago.VENT_DocsVenta.ENTID_CodigoCliente
   ...
   Private Sub acTool_ACBtnAnular_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles acTool.ACBtnAnular_Click
      Try
         If ACControles.ACDialogos.ACMostrarMensajePregunta(String.Format("Anular Registro: {0}", Me.Text) _
            ...
            Dim _docspagos As New BTESO_DocsPagos(m_puntoventa, GApp.Periodo, GApp.Zona, GApp.Sucursal)
            ...
            _docspagos.TESO_DocsPagos.DPAGO_Estado = ACEVentas.Constantes.getEstado(ACEVentas.Constantes.Estado.Anulado)
   ...
   Private Sub acTool_ACBtnGrabar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles acTool.ACBtnGrabar_Click
      Try
         ...
         If tabCheque.Selected Then
            ...
            m_edocpago.TIPOS_CodTipoMoneda = cmbChMoneda.SelectedValue
            m_edocpago.TIPOS_TipoMoneda = cmbChMoneda.Text
         ...
         m_edocpago.VENT_DocsVenta.SUCUR_Id = GApp.Sucursal
            m_edocpago.VENT_DocsVenta.PVENT_Id = m_puntoventa
         ...
         m_edocpago.DPAGO_Numero = actxnNumero.Text
         ...
         End If
         End If
         m_edocpago.PVENT_Id = m_puntoventa
         Dim _where As New Hashtable
         _where.Add("PVENT_Id", New ACWhere(m_puntoventa))
         m_edocpago.DPAGO_Id = managerBDocsPago.getCorrelativo("DPAGO_Id", _where)
         ...
         If m_dialogo = TipoInicio.Dialogo Then
            ...
            Me.Close()
         Else
            If tpgNotaCredito.Selected Then
               If managerBDocsPago.Guardar(GApp.Usuario, True, m_opcion) Then
         ...
         If e.X > c1grdBusqueda.Rows.Fixed Then
            ...
            Select m_opcion
               Case ETipos.TipoDocPago.Cheque
                  carga = True
               Case ETipos.TipoDocPago.Deposito
                  If CType(bs_docsPago.Current, ETESO_DocsPagos).DPAGO_Estado = ACEVentas.Constantes.getEstado(ACEVentas.Constantes.Estado.Anulado) Then
   ...
   Private Sub actxaCliRuc_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles actxaCliRuc.KeyDown
      Try
         If e.KeyData = Keys.Enter Then
            If actxaCliRuc.Text.ToString().Length >= Constantes.LongitudCodigo Then
               '' Cargar datos adicionales cliente
               If actxaCliRuc.ACAyuda.Enabled = True Then
                  If managerEntidades.CargarDocIden(actxaCliRuc.Text, EEntidades.TipoInicializacion.Cliente) Then
                     '' Cargar datos del cliente
                     m_edocpago.VENT_DocsVenta.ENTID_CodigoCliente = managerEntidades.Entidades.ENTID_Codigo
                     m_edocpago.ENTID_Codigo = managerEntidades.Entidades.ENTID_Codigo
                     actxaCliRazonSocial.Text = managerEntidades.Entidades.ENTID_RazonSocial
                     actxaCliRuc.Text = managerEntidades.Entidades.ENTID_NroDocumento
                     'actxnPPercepcion.Text = managerEntidades.Cliente.Percepcion
                     Label33.Focus()
                  Else
...
#End Region
   ...
   Private Sub tabMantenimiento_SelectionChanged(ByVal sender As Crownwood.DotNetMagic.Controls.TabControl, ByVal oldPage As Crownwood.DotNetMagic.Controls.TabPage, ByVal newPage As Crownwood.DotNetMagic.Controls.TabPage) Handles tabMantenimiento.SelectionChanged
