...
Public Class TReqCompras

#Region " Variables "

   Private managerGenerarRequiCompra As BGenerarRequiCompra
   Private managerConsultaArticulos As BConsultaArticulos
   Private managerEntidades As BEntidades
   '
   Private managerABAS_RequisicionesCompra As BABAS_CotizacionesCompra

   Private m_listBindHelper As List(Of ACBindHelper)

   '
   Private m_eabas_requisicionescompra As EABAS_CotizacionesCompra

   Private m_earticulos As EArticulos
   Private m_eentidades As EEntidades

   Private bs_requisicionescompras As BindingSource
   Private bs_detrequisicionescompras As BindingSource
...
#Region " Propiedades "
   Public Property ABAS_Cotizacioncompra() As EABAS_CotizacionesCompra
      Get
         Return m_eabas_requisicionescompra
      End Get
      Set(ByVal value As EABAS_CotizacionesCompra)
         m_eabas_requisicionescompra = value
      End Set
   End Property
#End Region
...
#Region " Constructores "
   ...
   Private Sub Inicializacion()
      Try
         ...
         managerGenerarRequiCompra = New BGenerarRequiCompra
         managerEntidades = New BEntidades
         managerConsultaArticulos = New BConsultaArticulos(GApp.Periodo)
         managerABAS_RequisicionesCompra = New BABAS_CotizacionesCompra
...
#Region " Metodos "
   ...
   Private Sub subirItem()
      Try
         If IsNothing(CType(bs_detrequisicionescompras.Current, EABAS_CotizacionesCompraDetalle).Articulo) Then
            '' Cargar Producto
            actxaProdCodigo.Text = CType(bs_detrequisicionescompras.Current, EABAS_CotizacionesCompraDetalle).ARTIC_Codigo
            txtProdDescripcion.Text = CType(bs_detrequisicionescompras.Current, EABAS_CotizacionesCompraDetalle).ARTIC_Descripcion
            actxnProdCantidad.Text = CType(bs_detrequisicionescompras.Current, EABAS_CotizacionesCompraDetalle).COTCD_Cantidad
            m_earticulos = New EArticulos()
            '' Asignar Metodos a controles
         Else
            m_earticulos = CType(bs_detrequisicionescompras.Current, EABAS_CotizacionesCompraDetalle).Articulo
            setProducto(True)
            actxnProdCantidad.Text = CType(bs_detrequisicionescompras.Current, EABAS_CotizacionesCompraDetalle).COTCD_Cantidad
         End If
      Catch ex As Exception
   ...
   Private Function addDetalle() As Boolean
      Try
         ...
         _filter.ACFiltrar(m_eabas_requisicionescompra.ListABAS_CotizacionesCompraDetalle)
         If _filter.ACListaFiltrada.Count > 0 Then
            If ACControles.ACDialogos.ACMostrarMensajePregunta(String.Format("Registro Duplicado: {0}", Me.Text) _
             ...
             , ACControles.ACDialogos.LabelBotom.Si_No) = DialogResult.Yes Then
               ...
               bs_detrequisicionescompras.ResetBindings(False)
               Return True
            End If
         Else
            ...
            If _detalle.COTCD_Cantidad > 0 Then
               ...
               m_eabas_requisicionescompra.ListABAS_CotizacionesCompraDetalle.Add(_detalle)
               bs_detrequisicionescompras.ResetBindings(False)
               Return True
            Else
...
#Region "Metodos Utilitarios"
   ...
   ''' <summary>
   ''' validar los datos de la requisicion de compra para inicial el procesa de grabado en la base de datos
   ''' </summary>
   ...
   Private Function validar(ByRef m_msg As String)
      Try
         ...
         If Not m_eabas_requisicionescompra.ListABAS_CotizacionesCompraDetalle.Count > 0 Then
   ...
   ''' <summary>
   ''' Validar el detalle del documento requisicion de compra,valida si se han ingresado las cantidades adecuadamente
   ''' </summary>
   ...
   Private Function ValidarDetalle(ByRef m_msg As String) As Boolean
      Try
         ...
         For Each Item As EABAS_CotizacionesCompraDetalle In m_eabas_requisicionescompra.ListABAS_CotizacionesCompraDetalle
   ...
   Private Sub formatearGrilla()
      ...
      Try
         ...
         ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdBusqueda, index, "Fecha", "COTCO_FechaDocumento", "COTCO_FechaDocumento", 150, True, False, "System.DateTime") : index += 1
         ...
         ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdDetalle, index, "Descripción", "ARTIC_Descripcion", "ARTIC_Descripcion", 530, True, True, "System.String") : index += 1
...
#Region "Cargar Datos"
   ...
   Private Sub cargarDatos()
      Try
         bs_requisicionescompras = New BindingSource()
         bs_requisicionescompras.DataSource = managerABAS_RequisicionesCompra.ListABAS_CotizacionesCompra
         c1grdBusqueda.DataSource = bs_requisicionescompras
         bnavBusqueda.BindingSource = bs_requisicionescompras
         AddHandler bs_requisicionescompras.CurrentChanged, AddressOf bs_requisiciones_CurrentChanged
         bs_requisiciones_CurrentChanged(Nothing, Nothing)
      Catch ex As Exception
         Throw ex
      End Try
   End Sub
   ...
   Private Sub AsignarBinding()
      Try
         ...
         m_listBindHelper.Add(ACBindHelper.ACBind(txtCodigo, "Text", m_eabas_requisicionescompra, "COTCO_Codigo"))
         If m_eabas_requisicionescompra.COTCO_FechaDocumento.Year < 1700 Then m_eabas_requisicionescompra.COTCO_FechaDocumento = DateTime.Now
         m_listBindHelper.Add(ACBindHelper.ACBind(dtpFecEmision, "Value", m_eabas_requisicionescompra, "COTCO_FechaDocumento"))
         m_listBindHelper.Add(ACBindHelper.ACBind(txtDireccion, "Text", m_eabas_requisicionescompra, "COTCO_DireccionProveedor"))
         m_listBindHelper.Add(ACBindHelper.ACBind(txtTelefono, "Text", m_eabas_requisicionescompra, "COTCO_TelefonoProveedor"))
         m_listBindHelper.Add(ACBindHelper.ACBind(txtEMail, "Text", m_eabas_requisicionescompra, "COTCO_CorreoProveedor"))
         m_listBindHelper.Add(ACBindHelper.ACBind(txtAtencion, "Text", m_eabas_requisicionescompra, "COTCO_AtencionProveedor"))
         ''PEDID_SubTotal
      Catch ex As Exception
         Throw ex
      End Try
         ...
         If Ayuda.ShowDialog() = System.Windows.Forms.DialogResult.OK Then
            Select Case x_opcion
               Case EEntidades.TipoEntidad.Proveedores
                  ...
                  m_eabas_requisicionescompra.ENTID_CodigoProveedor = Ayuda.Respuesta.Rows(0)("Codigo")
                  If managerEntidades.Cargar(m_eabas_requisicionescompra.ENTID_CodigoProveedor, EEntidades.TipoInicializacion.Proveedor) Then
...
#Region "Procesos"
   ...
   ''' <summary>
   ''' Realizar la busqueda de requisiciones de compra
   ''' </summary>
   ''' <param name="x_cadena">Cadena de busqueda de requisiciones</param>
   ''' <returns></returns>
   ''' <remarks></remarks>
   Private Function busqueda(ByVal x_cadena As String) As Boolean
      Try
         ...
         If managerABAS_RequisicionesCompra.Busqueda(GApp.Zona, GApp.Sucursal, x_cadena, getCampo(), chkTodos.Checked, AcFecha.ACDtpFecha_De.Value.Date, AcFecha.ACDtpFecha_A.Value.Date) Then
   ...
   ''' <summary>
   ''' Grabar la requisicion de compra en la base de datos
   ''' </summary>
   ''' <param name="m_msg">Mensaje devuelto en caso de ocurrir alguna incidencia</param>
   ''' <returns></returns>
   ''' <remarks></remarks>
   Private Function grabarRequiCompra(ByRef m_msg As String) As Boolean
      Try
         If validar(m_msg) Then
            If ACControles.ACDialogos.ACMostrarMensajePregunta(String.Format("Grabar Requisición de Compra: {0}", Me.Text) _
             , "Desea grabar la Requisición de Compra?" _
             , ACControles.ACDialogos.LabelBotom.Si_No) = DialogResult.Yes Then
               '' Obtener datos
               m_eabas_requisicionescompra.ZONAS_Codigo = GApp.Zona
               m_eabas_requisicionescompra.SUCUR_Id = GApp.Sucursal
               m_eabas_requisicionescompra.ALMAC_Id = GApp.Almacen
               '' Obtener el correlativo y asignarlo
               Dim x_corre As Integer = 0
               Dim x_tipo As String = EABAS_CotizacionesCompra.TipoPedido.RQ.ToString()
               m_eabas_requisicionescompra.COTCO_Codigo = managerABAS_RequisicionesCompra.getCorrelativo(x_tipo, x_corre)
               m_eabas_requisicionescompra.COTCO_Numero = x_corre
               'm_eabas_requisicionescompra.COTCO_Codigo = managerABAS_RequisicionesCompra.getCorrelativo(x_corre, GApp.PuntoVenta)
               '' Definir el Estado
               m_eabas_requisicionescompra.COTCO_Estado = EABAS_CotizacionesCompra.getEstado(EABAS_CotizacionesCompra.Estado.Ingresado)
               '' Asignar la clase pedido a la capa de negocio 
               managerGenerarRequiCompra.EABAST_CotizacionesCompra = m_eabas_requisicionescompra

               ''Asignar correlativo COTCO_Id
               If m_eabas_requisicionescompra.Nuevo = True Then
                  ''Correlativo
                  m_eabas_requisicionescompra.COTCO_Id = managerABAS_RequisicionesCompra.getCorrelativo("COTCO_ID")
                  managerABAS_RequisicionesCompra.ABAS_CotizacionesCompra = m_eabas_requisicionescompra
               End If

               '' Grabar el pedido
               If managerGenerarRequiCompra.GrabaRequiCompra(GApp.Usuario, m_msg) Then
                  Return True
               Else
   ...
   ''' <summary>
   ''' Crear nueva requisicion de compra
   ''' </summary>
   ''' <remarks></remarks>
   Private Sub nuevaRequisicion()
      Try
         '' Inicializar clase Pedido
         m_eabas_requisicionescompra = New EABAS_CotizacionesCompra
         m_eabas_requisicionescompra.Instanciar(ACEInstancia.Nuevo)
         ' '' Instanciar clase
         AsignarBinding()
         setInstancia(ACUtilitarios.ACSetInstancia.Nuevo)
         '' Para Cargar detalle del producto
         bs_detrequisicionescompras = New BindingSource
         m_eabas_requisicionescompra.ListABAS_CotizacionesCompraDetalle = New List(Of EABAS_CotizacionesCompraDetalle)
         bs_detrequisicionescompras.DataSource = m_eabas_requisicionescompra.ListABAS_CotizacionesCompraDetalle
         c1grdDetalle.DataSource = bs_detrequisicionescompras
         bnavProductos.BindingSource = bs_detrequisicionescompras
   ...
   ''' <summary>
   ''' Ordenar la grilla de requisiciones de compra
   ''' </summary>
   ...
   Private Sub OrdenarPedidos(ByVal x_columna As String)
      ...
      Try
         ...
         CType(bs_requisicionescompras.DataSource, List(Of EABAS_CotizacionesCompra)).Sort(_ordenador)
...
#Region " Metodos de Controles"
   ...
   Private Sub acbtnSeleccionar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles acbtnSeleccionar.Click
      Try
         If Not IsNothing(bs_requisicionescompras) Then
            If Not IsNothing(bs_requisicionescompras.Current) Then
               m_eabas_requisicionescompra = CType(bs_requisicionescompras.Current, EABAS_CotizacionesCompra)
               Me.DialogResult = System.Windows.Forms.DialogResult.OK
               Me.Close()
            Else
               ACControles.ACDialogos.ACMostrarMensajeInformacion(String.Format("Información: {0}", Me.Text), "No se puede continuar por que no ha seleccionado ninguna requisición de compra")
            End If
         Else
            ACControles.ACDialogos.ACMostrarMensajeInformacion(String.Format("Información: {0}", Me.Text), "No se puede continuar por que no ha seleccionado ninguna requisición de compra")
         End If
      Catch ex As Exception
         ACControles.ACDialogos.ACMostrarMensajeError(String.Format("Error: {0}", Me.Text), "Ocurrio un error en el proceso seleccionar Requisicion", ex)
      End Try
   End Sub
   ...
   Private Sub btnNuevoProveedor_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNuevoProveedor.Click
      Try
         ...
         If frmEntidad.ShowDialog() = System.Windows.Forms.DialogResult.OK Then
            ...
            m_eabas_requisicionescompra.ENTID_CodigoProveedor = m_eentidades.ENTID_Codigo
            actxaProvCodigo.Text = managerEntidades.Entidades.ENTID_Codigo
            actxaProvRazonSocial.Text = managerEntidades.Entidades.ENTID_RazonSocial
            actxaProvRuc.Text = managerEntidades.Entidades.ENTID_NroDocumento
   ...
   Private Sub bs_requisiciones_CurrentChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
      Try
         If Not IsNothing(bs_requisicionescompras.Current) Then
            Select Case CType(bs_requisicionescompras.Current, EABAS_CotizacionesCompra).COTCO_Estado
         ...
         End If
      Catch ex As Exception
         ACControles.ACDialogos.ACMostrarMensajeError(String.Format("Error: {0}", Me.Text), "Ocurrio un error en el proceso Cargar Propiedades de Requisición/Pedido", ex)
      End Try
   End Sub
...
#Region "ToolBar"
   ...
   Private Sub actsbtnPrevisualizar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles actsbtnPrevisualizar.Click
      Try
         Dim x_codigo As String = CType(bs_requisicionescompras.Current, EABAS_CotizacionesCompra).COTCO_Codigo
         '' Cargar Requisicion
         If managerABAS_RequisicionesCompra.Cargar(x_codigo, True) Then
            m_eabas_requisicionescompra = managerABAS_RequisicionesCompra.ABAS_CotizacionesCompra
            AsignarBinding()
            ...
            tabMantenimiento.SelectedTab = tabDatos
            '' Cargar datos del Cliente
            If managerEntidades.Cargar(m_eabas_requisicionescompra.ENTID_CodigoProveedor) Then
               actxaProvCodigo.Text = m_eabas_requisicionescompra.ENTID_CodigoProveedor
               actxaProvRuc.Text = m_eabas_requisicionescompra.ENTID_NroDocumento
               actxaProvRazonSocial.Text = managerEntidades.Entidades.ENTID_RazonSocial
            End If
   ...
   Private Sub acTool_ACBtnNuevo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles acTool.ACBtnNuevo_Click
      ...
      Catch ex As Exception
         ...
         ACControles.ACDialogos.ACMostrarMensajeError(String.Format("Error: {0}", Me.Text), "Ocurrio un error en el proceso Nueva Requisición/Pedido", ex)
      End Try
   End Sub
   ...
   Private Sub acTool_ACBtnGrabar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles acTool.ACBtnGrabar_Click
      Dim m_msg As String = ""
      Try
         If grabarRequiCompra(m_msg) Then
      ...
      Catch ex As Exception
         acTool.setInstancia(ACControles.ACToolBarMantHorizontalNew.TipoInstancia.Nuevo)
         ACControles.ACDialogos.ACMostrarMensajeError(String.Format("Error: {0}", Me.Text), "Ocurrio un error en el proceso Grabar Requisición/Pedido", ex)
      End Try
   End Sub
   ...
   Private Sub acTool_ACBtnCancelar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles acTool.ACBtnCancelar_Click, acTool.ACBtnBuscar_Click
      Try
         If pnlItem.Enabled Then
            If m_eabas_requisicionescompra.ListABAS_CotizacionesCompraDetalle.Count > 0 Then
               If ACControles.ACDialogos.ACMostrarMensajePregunta(String.Format("Cancelar Pedido/Requisición: {0}", Me.Text) _
         ...
         End If
      Catch ex As Exception
         ACControles.ACDialogos.ACMostrarMensajeError(String.Format("Error: {0}", Me.Text), "Ocurrio un error en el proceso cancelar la Requisición/Pedido", ex)
      End Try
   End Sub
   ...
   Private Sub acTool_ACBtnAnular_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles acTool.ACBtnAnular_Click
      Try
         If ACControles.ACDialogos.ACMostrarMensajePregunta(String.Format("Anular Requisición: {0}", Me.Text) _
            , String.Format("Desea Anular la Requisición: {0}?", CType(bs_requisicionescompras.Current, EABAS_CotizacionesCompra).COTCO_Codigo) _
            , ACControles.ACDialogos.LabelBotom.Si_No) = DialogResult.Yes Then
            tabMantenimiento.SelectedTab = tabBusqueda
            acTool.ACBtnModificarVisible = m_proceso_modificar
            setInstancia(ACUtilitarios.ACSetInstancia.Deshacer)
            managerABAS_RequisicionesCompra.ABAS_CotizacionesCompra = New EABAS_CotizacionesCompra()
            managerABAS_RequisicionesCompra.ABAS_CotizacionesCompra.COTCO_Codigo = CType(bs_requisicionescompras.Current, EABAS_CotizacionesCompra).COTCO_Codigo
            managerABAS_RequisicionesCompra.ABAS_CotizacionesCompra.COTCO_Estado = EABAS_CotizacionesCompra.getEstado(EABAS_CotizacionesCompra.Estado.Anulado)
            managerABAS_RequisicionesCompra.Guardar(GApp.Usuario)
            txtBusqueda_ACAyudaClick(Nothing, Nothing)
         Else
            acTool.setInstancia(ACControles.ACToolBarMantHorizontalNew.TipoInstancia.Nuevo)
         End If
      Catch ex As Exception
         ACControles.ACDialogos.ACMostrarMensajeError(String.Format("Error: {0}", Me.Text), "Ocurrio un error en el proceso Anular Requisición", ex)
      End Try
   End Sub
...
#Region "Ayuda para los proveedores"
      ...
      Try
         If e.KeyData = Keys.Enter And actxaProvCodigo.Text.ToString().Length >= Constantes.LongitudCodigo Then
            ...
            If managerEntidades.Cargar(actxaProvCodigo.Text, EEntidades.TipoInicializacion.Cliente) Then
               '' Cargar datos del cliente
               m_eabas_requisicionescompra.ENTID_CodigoProveedor = managerEntidades.Entidades.ENTID_Codigo
               m_eentidades = managerEntidades.Entidades
               actxaProvCodigo.Text = managerEntidades.Entidades.ENTID_Codigo
               actxaProvRazonSocial.Text = managerEntidades.Entidades.ENTID_RazonSocial
               actxaProvRuc.Text = managerEntidades.Entidades.ENTID_NroDocumento
   ...
   Private Sub actxaProvRuc_ACAyudaClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles actxaProvRuc.ACAyudaClick
      Try
         AyudaEntidad(actxaProvCodigo.Text, "ENTID_NroDocumento", EEntidades.TipoEntidad.Proveedores)
      Catch ex As Exception
...
#Region "Ayudas"
   ...
   Private Sub txtBusqueda_ACAyudaClick(ByVal sender As Object, ByVal e As EventArgs) Handles txtBusqueda.ACAyudaClick, btnConsultar.Click
      ...
      Catch ex As Exception
         ACControles.ACDialogos.ACMostrarMensajeError(String.Format("Error: {0}", Convert.ToString(Text)), "No se puede cargar la ayuda para las requisiciones", ex)
      End Try
   End Sub
         ...
         Select Case e.KeyChar
            Case "+"c
               If m_modArticulo Then
                  e.Handled = True
                  CType(bs_detrequisicionescompras.Current, EABAS_CotizacionesCompraDetalle).COTCD_Cantidad = actxnProdCantidad.Text
                  setProducto(False)
   ...
   Private Sub txtBusqueda_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtBusqueda.KeyDown
      Try
         Select Case e.KeyCode
            Case Keys.Down
               bs_requisicionescompras.Position += 1
               e.Handled = True
               Exit Select
            Case Keys.Up
               bs_requisicionescompras.Position -= 1
               ...
               Exit Select
            Case Keys.PageDown
               bs_requisicionescompras.Position += 10
               e.Handled = True
               Exit Select
            Case Keys.PageUp
               bs_requisicionescompras.Position -= 10
...
#Region "Grillas" ''ojo revisar al momento de guardar en la BD
      ...
      Try
         ...
         Select Case e.KeyCode
            ...
            Case Keys.Delete
             ...
             , String.Format("Desea quitar el Articulo : {0} ", CType(bs_detrequisicionescompras.Current, EVENT_PedidosDetalle).ARTIC_Descripcion) _
             , ACControles.ACDialogos.LabelBotom.Si_No) = DialogResult.Yes Then
                  Dim m_det As EABAS_CotizacionesCompraDetalle = CType(bs_detrequisicionescompras.Current, EABAS_CotizacionesCompraDetalle)
                  m_eabas_requisicionescompra.ListABAS_CotizacionesCompraDetalle.Remove(m_det)
                  bs_detrequisicionescompras.ResetBindings(False)
               End If
            Case Else
...
Private Sub TReqCompras_Load( sender As Object,  e As EventArgs) Handles MyBase.Load
