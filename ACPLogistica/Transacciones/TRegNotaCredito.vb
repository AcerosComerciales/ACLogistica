...
#Region " Utilitarios "
   ...
   Private Sub formatearGrilla()
      ...
      Try
         ...
         ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdBusqueda, index, "Doc. Referencia", "DOCCO_Referencia", "DOCCO_Referencia", 150, True, False, "System.String") : index += 1
         ...
         ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdDetalle, index, "Costo", "Costo", "Costo", 75, False, False, "System.Decimal", Parametros.GetParametro(EParametros.TipoParametros.pg_FMondo2d)) : index += 1
...
#Region " Procesos "
   ...
   Private Function busqueda(ByVal x_cadena As String, Optional ByVal x_documento As Boolean = False) As Boolean
      Try
         Dim x_in As String = getIn()
         If x_documento Then
            If managerABAS_DocsCompra.Busqueda(cmbTipoDocumento.SelectedValue, txtCSerie.Text, txtBusNumero.Text, chkTodos.Checked, acFecha.ACDtpFecha_De.Value.Date, acFecha.ACDtpFecha_A.Value.Date.AddDays(1), x_in) Then
         ...
         Else
            ...
            If managerABAS_DocsCompra.Busqueda(x_cadena, getCampo(), chkTodos.Checked, acFecha.ACDtpFecha_De.Value.Date, acFecha.ACDtpFecha_A.Value.Date.AddDays(1), x_in) Then
               acTool.ACBtnEliminarEnabled = True
               acTool.ACBtnModificarEnabled = True
            Else
               acTool.ACBtnEliminarEnabled = False
      ...
      Try
         If c1grdDetalle.Cols(e.Col).Name = "DOCCD_Cantidad" And e.Row > (c1grdDetalle.Rows.Fixed - 1) Then
            If IsNumeric(c1grdDetalle.Rows(e.Row)(e.Col).ToString()) Then
               ...
               Dim _original As Double = c1grdDetalle.Rows(e.Row)("Costo")
               If _cantidad > _original Or _cantidad < 0 Then
                  c1grdDetalle.Rows(e.Row)("DOCCD_Cantidad") = c1grdDetalle.Rows(e.Row)("Costo")
   ...
   Private Sub actxaDocReferencia_ACAyudaClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles actxaDocReferencia.ACAyudaClick
      Try
         If rbtnDocRefVenta.Checked Then
            ...
            If frmCompras.ShowDialog() = System.Windows.Forms.DialogResult.OK Then
               setInstancia(ACUtilitarios.ACSetInstancia.Nuevo)
               ...
               bnavProductos.BindingSource = bs_detdocscompra
               cmbTipoPago.Tag = ""
               cmbTipoPago.SelectedValue = m_eabas_docscompra.TIPOS_CodTipoPago
