        ...
        Try
            ...
            Select Case _opcion 'ESTO SE USA MAS ADELANTE PARA CUANDO SE PASA UNA INSTANCIA DE NUEVO ETC FRANK 
                ...
                Case ACFramework.ACUtilitarios.ACSetInstancia.Modificado
                    ...
                    '        Case Procesos.getProceso(Procesos.TipoProcesos.Abastecimientos_AnularCotizacionCompra)
                    '            x_eliminar = True
...
#Region " Metodos "
    ...
    ''' </summary>
    ''' <param name="x_eabas_pedidoscompra"></param>
    ''' <remarks></remarks>
    Private Sub PedidoCompraToCotizacion(ByVal x_eabas_pedidoscompra As EABAS_PedidosCompra)
      Try
         Dim x_codigo As String = x_eabas_pedidoscompra.PEDID_Codigo
         '' Cargar Cotizacion
         If managerABAS_PedidosCompra.Cargar(x_codigo, True) Then
            '' Setear Valores
            setInstancia(ACUtilitarios.ACSetInstancia.Nuevo)
            '' Cargar los datos de la cabecera
            Dim m_pedidoc As EABAS_PedidosCompra = managerABAS_PedidosCompra.ABAS_PedidosCompra
           '- m_eabas_ordenescompra.ENTID_CodigoProveedor = m_pedidoc.ENTID_CodigoProveedor
           ' m_eabas_ordenescompra.ORDCO_DireccionProveedor = m_pedidoc.COTCO_DireccionProveedor
           ' m_eabas_ordenescompra.ORDCO_AtencionProveedor = m_pedidoc.COTCO_AtencionProveedor
           ' m_eabas_ordenescompra.ORDCO_TelefonoProveedor = m_pedidoc.COTCO_TelefonoProveedor
           ' m_eabas_ordenescompra.ORDCO_CorreoProveedor = m_pedidoc.COTCO_CorreoProveedor
            m_eabas_cotizacionescompra.PEDIDCO_Codigo = m_pedidoc.PEDID_Codigo
            AsignarBinding()
            ...
            For Each Item As EABAS_PedidosCompraDetalle In m_pedidoc.ListDetPedidosCompra
               Dim detalle As New EABAS_CotizacionesCompraDetalle()
               detalle.ARTIC_Codigo = Item.ARTIC_Codigo
               detalle.COTCD_Cantidad = Item.PDDET_Cantidad
               If managerConsultaArticulos.cargarProducto(Item.ARTIC_Codigo) Then
                  detalle.Articulo = managerConsultaArticulos.Articulos
                  detalle.COTCD_PesoUnitario = detalle.Articulo.ARTIC_Peso
               ...
               End If
               'If managerABAS_ListaPreciosCompra.GetPreciosArticulo(m_pedidoc.ENTID_CodigoProveedor, Item.ARTIC_Codigo) Then
               ...
               'End If
               m_eabas_cotizacionescompra.ListABAS_CotizacionesCompraDetalle.Add(detalle)
            Next
            calcular()
            ...
            actxaNroPedidoCompra.Enabled = False
      ...
      Try
         ...
         Else
            ...
            _detalle.COTCD_Cantidad = actxnProdCantidad.Text
                _detalle.COTCD_PesoUnitario = m_earticulos.ARTIC_Peso
                 _detalle.TIPOS_UnidadMedida = m_earticulos.TIPOS_UnidadMedida
            _detalle.ARTIC_Codigo = m_earticulos.ARTIC_Codigo
...
#Region " Metodos Utilitarios "
   ...
   Private Function ValidarDetalle(ByRef m_msg As String) As Boolean
      Try
         ...
         For Each Item As EABAS_CotizacionesCompraDetalle In m_eabas_cotizacionescompra.ListABAS_CotizacionesCompraDetalle
                If Not Item.COTCD_Cantidad > 0.1 Then
               ...
               _filter.ACFiltro = String.Format("TIPOS_Codigo={0}", m_earticulos.TIPOS_CodUnidadMedida)
                    If Not _filter.ACFiltrar(Colecciones.TiposUniMedDecimal).Count > 0.1 Then
   ...
   End Function
      ...
      Try
         If Not IsNothing(m_eabas_cotizacionescompra) Then
            If Not IsNothing(m_eabas_cotizacionescompra.ListABAS_CotizacionesCompraDetalle) Then
               If m_eabas_cotizacionescompra.ListABAS_CotizacionesCompraDetalle.Count > 0 Then
                  ...
                  Dim _cantidad As Double = 0
                    For Each Item As EABAS_CotizacionesCompraDetalle In m_eabas_cotizacionescompra.ListABAS_CotizacionesCompraDetalle
                         _pesoTotal += Item.COTCD_PesoUnitario * Item.COTCD_Cantidad
                              Next
   ...
   Private Sub formatearGrilla()
      ...
      Try
        ...
        ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdDetalle, index, "Unidad", "TIPOS_UnidadMedida", "TIPOS_UnidadMedida", 90, True, False, "System.String") : index += 1
         ACFrameworkC1.ACUtilitarios.ACAgregarColumna(c1grdDetalle, index, "Peso", "COTCD_PesoUnitario", "PDDET_PesoUnitario", 100, True, False, "System.Decimal", Parametros.GetParametro(EParametros.TipoParametros.pg_FMondo4d)) : index += 1
        ...
        Select Case _opcion
         Case ACUtilitarios.ACSetInstancia.Nuevo
            ...
            ACUtilitarios.ACLimpiaVar(pnlItem)
                ACUtilitarios.ACLimpiaVar(txtObservacion)
                actxaCotizacionProveedor.Text = ""
                txtObservacion.Clear()
            SetNuevo()
                actxaNroPedidoCompra.Enabled = True
         ...
         Case ACUtilitarios.ACSetInstancia.Previsualizar
                ...
                SetActivar(False) 'aqui el Parametros lo pasa Falso por ende no entra o Confirmar los permisos  ??? 
                setInstanciaPermisos(ACUtilitarios.ACSetInstancia.Previsualizar)
   ...
   End Sub
      ...
      Try
         ...
         m_modificando = False
          pnlItem.Enabled = true
         btnClean.Enabled = True
 ACUtilitarios.ACSetControl(pnlCabecera, Not True, ACUtilitarios.TipoPropiedad.ACReadOnly)
         
         ACUtilitarios.ACSetControl(GroupBox1, Not True, ACUtilitarios.TipoPropiedad.ACReadOnly)
         ...
         actxaProvRuc.ACActivarAyuda = True
            ...
            acTool.ACBtnAnularVisible = Not True
            ...
            'If m_tipoDocumento = EVENT_Pedidos.TipoPedido.PI Then
            '    acTool.ACBtnRehusarVisible = x_opcion
            'Else
            '    acTool.ACBtnRehusarVisible = Not x_opcion
            'End If
            '
         ...
         RemoveHandler c1grdDetalle.KeyDown, AddressOf c1grdDetalle_KeyDown
         AddHandler c1grdDetalle.KeyDown, AddressOf c1grdDetalle_KeyDown
            acTool.ACBtnAnularVisible = True ' false   '_M
   ...
   Private Sub SetActivar(ByVal x_opcion As Boolean)
      Try
         If x_opcion Then
            AddHandler c1grdDetalle.KeyDown, AddressOf c1grdDetalle_KeyDown
            acTool.setInstancia(ACControles.ACToolBarMantHorizontalNew.TipoInstancia.Nuevo)
         ...
         ACUtilitarios.ACSetControl(c1grdDetalle, Not x_opcion, ACUtilitarios.TipoPropiedad.ACReadOnly)
         ACUtilitarios.ACSetControl(GroupBox1, Not x_opcion, ACUtilitarios.TipoPropiedad.ACReadOnly)

         pnlItem.Enabled = x_opcion
         '' Vendedor
         actxaProvRazonSocial.ACAyuda.Enabled = x_opcion
         actxaProvRazonSocial.ACActivarAyuda = x_opcion
         actxaProvRuc.ACAyuda.Enabled = x_opcion
         actxaProvRuc.ACActivarAyuda = x_opcion
  
            acTool.ACBtnAnularVisible = Not x_opcion
            acTool.ACBtnAnularEnabled = False


         btnNuevoProveedor.Enabled = x_opcion

            acTool.ACBtnGrabarVisible = x_opcion
  
               ' acTool.ACBtnRehusarVisible = Not x_opcion
           
        

            actsbtnPrevisualizar.Visible = x_opcion
            acTool.ACBtnImprimirVisible = True
            btnClean.Enabled = x_opcion
   ...
   Private Sub AsignarBinding()
      Try
         ...
         m_listBindHelper.Add(ACBindHelper.ACBind(txtAtencion, "Text", m_eabas_cotizacionescompra, "COTCO_AtencionProveedor"))
            ...
            m_listBindHelper.Add(ACBindHelper.ACBind(actxaNroPedidoCompra,"Text",m_eabas_cotizacionescompra,"PEDIDCO_Codigo"))
         ''PEDID_SubTotal
            m_listBindHelper.Add(ACBindHelper.ACBind(actxnPesoTotal, "Text", m_eabas_cotizacionescompra, "COTCO_PesoTotal"))
              m_listBindHelper.Add(ACBindHelper.ACBind(txtObservacion, "Text",m_eabas_cotizacionescompra, "COTCO_Observaciones"))
      Catch ex As Exception
         Throw ex
      End Try
   End Sub
...
#Region " Para el Proveedor "
   ...
   ''' <remarks></remarks>
   Private Sub AyudaEntidad(ByVal x_cadenas As String, ByVal x_campo As String,ByVal x_descripcion As String, ByVal x_opcion As EEntidades.TipoEntidad)
      Try
         'Me.KeyPreview = False
             ...
             , ACControles.ACDialogos.LabelBotom.Si_No) = DialogResult.Yes Then
               If m_eabas_cotizacionescompra.Nuevo Then
                  ...
                  m_eabas_cotizacionescompra.COTCO_Codigo = managerABAS_CotizacionesCompra.getCorrelativo(x_tipo, x_corre, GApp.Almacen)
                        ...
                        'COTCO_CotizacionProveedor

                        'COTIZACION PROVEEDOR 
                        ' actxaCotizacionProveedor.Text = managerABAS_DocsCompra.ABAS_DocsCompra.DOCCO_CotizacionProveedor
                        m_eabas_cotizacionescompra.COTCO_CotizacionProveedor = actxaCotizacionProveedor.Text
                        '' Definir el Estado
   ...
   Private Sub acTool_ACBtnNuevo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles acTool.ACBtnNuevo_Click
      ...
      '   Me.KeyPreview = True
           Try
            nuevaCotizacion()
               frmCons = New CProductos(CProductos.Origen.Cotizaciones) With {.StartPosition = FormStartPosition.CenterScreen}
            actxaNroPedidoCompra.ReadOnly = False : actxaNroPedidoCompra.ACAyuda.Enabled = True : actxaNroPedidoCompra.ACActivarAyuda = True
            '' Activar las opciones generales del teclado
            KeyPreview = True
   ...
   Private Sub actxaProvRazonSocial_ACAyudaClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles actxaProvRazonSocial.ACAyudaClick
      Try
         'AyudaEntidad(actxaProvRazonSocial.Text, "ENTID_RazonSocial", EEntidades.TipoEntidad.ProveedoresLogistica)
            AyudaEntidad(actxaProvRazonSocial.Text, 1, "ENTID_RazonSocial", EEntidades.TipoEntidad.Proveedores)
      Catch ex As Exception
   ...
   Private Sub actxaProvRuc_ACAyudaClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles actxaProvRuc.ACAyudaClick
      Try
         'AyudaEntidad(actxaProvRuc.Text, "ENTID_NroDocumento", EEntidades.TipoEntidad.Proveedores)
            AyudaEntidad(actxaProvRuc.Text, 0, "Nro Documento", EEntidades.TipoEntidad.Proveedores)
      Catch ex As Exception
...
#Region " Ayudas "
   ...
   Private Sub txtBusqueda_ACAyudaClick(ByVal sender As Object, ByVal e As EventArgs) Handles txtBusqueda.ACAyudaClick, btnConsultar.Click
      ...
      End Try
   End Sub
     Private Sub actxaNroPedidoCompra_ACAyudaClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles actxaNroPedidoCompra.ACAyudaClick
      Try
         Dim frmPedidoCompra As New TPedidoCompra(TPedidoCompra.TInicio.Busqueda)
            frmPedidoCompra.StartPosition = FormStartPosition.CenterScreen

         If frmPedidoCompra.ShowDialog() = System.Windows.Forms.DialogResult.OK Then
            PedidoCompraToCotizacion(frmPedidoCompra.ABAS_Pedidoscompra)
            pnlItem.Enabled = False
         End If
      Catch ex As Exception
         ACControles.ACDialogos.ACMostrarMensajeError(String.Format("Error: {0}", Me.Text), "Ocurrio un error en el proceso cargar Pedidos de Compra", ex)
      End Try
   End Sub
      ...
      Try
         ...
         If frmCons.ShowDialog() = System.Windows.Forms.DialogResult.OK Then
            ...
            If _filter.ACFiltrar(Colecciones.TiposUniMedDecimal).Count > 0 Then
                ...
                Else
                    actxnProdCantidad.ACDecimales = 3
                End If
            Me.KeyPreview = True
         End If
      Catch ex As Exception
...
Private Sub TCotCompras_Load( sender As Object,  e As EventArgs) Handles MyBase.Load
