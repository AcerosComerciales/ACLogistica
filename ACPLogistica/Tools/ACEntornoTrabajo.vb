...
#Region " Constructores "
   Public Sub New()
      ...
      Try
         ...
         ACUtilitarios.ACCargaCombo(cmbPeriodo, Colecciones.ListPeriodos, "PERIO_Descripcion", "PERIO_Codigo", GApp.Periodo)
         'lblPeriodo.Text = String.Format(lblPeriodo.Text, GApp.Periodo)
         ...
         If CType(Utilitarios.cConfig("zona"), PXML).Valor <> "" Then
            ...
            If CType(Utilitarios.cConfig("sucursal"), PXML).Valor <> "" Then
               ...
               If CType(Utilitarios.cConfig("almacen"), PXML).Valor <> "" Then
                  cmbAlmacen.SelectedValue = CType(CType(Utilitarios.cConfig("almacen"), PXML).Valor, Int16)
               End If
            End If
         End If
...
#Region " Metodos "
   Private Function validarUsuario(ByRef m_msg As String) As Boolean
      Try
         Dim managerVENT_UsuariosPorAlmacen As New BUsuariosPorAlmacen
         ...
         If managerVENT_UsuariosPorAlmacen.CargarTodos(_where) Then
            Return True
         Else
            m_msg &= String.Format("El usuario {0} no tiene acceso a: {4}- Zona: {1}{4} - Sucursal:{2}{4} - Almacen: {3}{4}Consulte con el administrador del sistema." _
                                   , New Object() {GApp.EUsuarioEntidad.ENTID_Nombres, cmbZona.Text, cmbSucursales.Text, cmbAlmacen.Text, vbNewLine})
            Return False
         End If
      Catch ex As Exception
         Throw ex
      ...
      Try
         If Not IsNothing(bs_zonas) Then
            If CType(bs_zonas.DataSource, List(Of EZonas)).Count > 0 Then
               ...
               Dim _filtAlma As New ACFiltrador(Of EAlmacenes)() With {.ACFiltro = String.Format("SUCUR_Id={0}", cmbSucursales.SelectedValue)}
               bs_almacenes.DataSource = _filtAlma.ACFiltrar(Colecciones.Almacenes)
               ACUtilitarios.ACCargaCombo(cmbAlmacen, bs_almacenes, "ALMAC_Descripcion", "ALMAC_Id")
...
#Region " Metodos de Controles"
   Private Sub btnAceptar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAceptar.Click
      Dim m_msg As String = ""
      Try
         If cmbZona.SelectedIndex > -1 And cmbSucursales.SelectedIndex > -1 And cmbAlmacen.SelectedIndex > -1 Then
            Dim _puntoventa As New EAlmacenes
            _puntoventa = CType(bs_almacenes.Current, EAlmacenes)
            If ACUtilitarios.ACCrearCadena("StrConn", _puntoventa.PVENT_DireccionIP, _puntoventa.PVENT_BaseDatos, GApp.Usuario_DB, GApp.Password_DB) Then
               DAEnterprise.Instanciar("StrConn", True)
               If validarUsuario(m_msg) Then
                        GApp.SetBase(CType(bs_zonas.Current, EZonas), CType(bs_sucursales.Current, ESucursales), CType(bs_almacenes.Current, EAlmacenes), dtpFecha.Value)
                  Dim _almacen As EAlmacenes = CType(cmbAlmacen.SelectedItem, EAlmacenes)
                  GApp.SetAcceso(_almacen.PVENT_DireccionIP, _almacen.PVENT_DireccionIPDesc, _almacen.PVENT_BaseDatos, _almacen.PVENT_DireccionIP, cmbPeriodo.SelectedValue)
                  GApp.DataConexion.BaseDatos = _almacen.PVENT_BDAdmin
                  Parametros.Inicializar(GApp.EZona.ZONAS_Codigo, GApp.ESucursal.SUCUR_Id, GApp.Aplicacion)
                  Colecciones.Inicializar(False)
                  ...
                  Utilitarios.cConfig("almacen") = New PXML(cmbAlmacen.SelectedValue, "almacen")
                  Utilitarios.saveXML()

                        GApp.cargarDatosUsuarios(GApp.Usuario, GApp.Aplicacion, GApp.Empresa)

                  '' Inicializar en la Capa de Negocio
                  Dim managerBConstantes As BConstantes
                  managerBConstantes = New BConstantes
                  managerBConstantes.setIniciales(GApp.Aplicacion, GApp.Zona, GApp.Sucursal, 0, GApp.Periodo, GApp.EUsuarioEntidad)
               ...
               End If
            Else
               ACControles.ACDialogos.ACMostrarMensajeInformacion(String.Format("Información: {0}", Me.Text), "No se puede continuar hasta que se selecciones la Zona/Sucursal/Almacen/Punto de Venta Adecuada ")
            End If
         Else
