...
#Region " Campos "
    ...
    Private m_orden_codigo_destino As String
    ...
    Private m_orden_codorigen As String
    ...
    Private m_artic_descripcion As String  'Esto se creo para la descripcion no existe en la bd 
...
#Region" Constructores "
	...
	Public Sub New()
		...
		Try
			Dim _obj As Object = ACELogistica.My.Resources.xmlDIST_OrdenesDetalle
...
#Region " Propiedades "
        ...
        Set(ByVal value As String)
            If Not IsNothing(m_docve_codigo) Then
                If Not m_docve_codigo.Equals(value) Then
                    ...
                    OnORDEN_CodigoChanged(m_docve_codigo, EventArgs.Empty)
                End If
            Else
                m_docve_codigo = value
                OnORDEN_CodigoChanged(m_docve_codigo, EventArgs.Empty)
    ...
    Public Property ORDEN_Codorigen() As String
        Get
            Return m_orden_codorigen
        End Get
        Set(ByVal value As String)
            If Not IsNothing(m_orden_codorigen) Then
                If Not m_orden_codorigen.Equals(value) Then
                    m_orden_codorigen = value
                    OnORDEN_CodorigenChanged(m_orden_codorigen, EventArgs.Empty)
                End If
            Else
                m_orden_codorigen = value
                OnORDEN_CodorigenChanged(m_orden_codorigen, EventArgs.Empty)
    ...
    Public Property ORDEN_Codigo_Destino() As String
        Get
            Return m_orden_codigo_destino
        End Get
        Set(ByVal value As String)
            If Not IsNothing(m_orden_codigo_destino) Then
                If Not m_orden_codigo_destino.Equals(value) Then
                    m_orden_codigo_destino = value
                    OnORDEN_CodigoChanged(m_orden_codigo_destino, EventArgs.Empty)
                End If
            Else
                m_orden_codigo_destino = value
                OnORDEN_CodigoChanged(m_orden_codigo_destino, EventArgs.Empty)
        ...
        Set(ByVal value As Decimal)
            If Not IsNothing(m_docve_codigo) Then
                If Not m_diferencia.Equals(value) Then
                    m_diferencia = value
                    OnDiferenciaChanged(m_diferencia, EventArgs.Empty)
                End If
            Else
                m_diferencia = value
                OnDiferenciaChanged(m_diferencia, EventArgs.Empty)
        ...
        Set(ByVal value As String)
            If Not IsNothing(m_artic_descripcion) Then
                If Not m_orden_codigo.Equals(value) Then
                    ...
                    OnORDEN_CodigoChanged(m_artic_descripcion, EventArgs.Empty)
                End If
            Else
                m_artic_descripcion = value
                OnORDEN_CodigoChanged(m_artic_descripcion, EventArgs.Empty)
   ...
   End Property
        ...
        Set(ByVal value As Decimal)
            If Not IsNothing(m_ordet_faltante) Then
                If Not m_ordet_faltante.Equals(value) Then
                    ...
                    OnORDEN_CodigoChanged(m_ordet_faltante, EventArgs.Empty)
                End If
            Else
                m_ordet_faltante = value
                OnORDEN_CodigoChanged(m_ordet_faltante, EventArgs.Empty)
        ...
        Set(ByVal value As Decimal)
            If Not IsNothing(m_ordet_stockdestino) Then
                If Not m_ordet_stockdestino.Equals(value) Then
                    ...
                    OnORDEN_CodigoChanged(m_ordet_stockdestino, EventArgs.Empty)
                End If
            Else
                m_ordet_stockdestino = value
                OnORDEN_CodigoChanged(m_ordet_stockdestino, EventArgs.Empty)
    ...
    'SAAAAAA
    Public Property ORDET_FecAlmacen() As DateTime
        Get
            Return m_ordet_fecalmacen
        End Get
        Set(ByVal value As DateTime)
            If Not IsNothing(m_ordet_fecalmacen) Then
                If Not m_ordet_fecalmacen.Equals(value) Then
                    m_ordet_fecalmacen = value
                    OnORDEN_CodigoChanged(m_ordet_fecalmacen, EventArgs.Empty)
                End If
            Else
                m_ordet_fecalmacen = value
                OnORDEN_CodigoChanged(m_ordet_fecalmacen, EventArgs.Empty)
        ...
        Set(ByVal value As Decimal)
            If Not IsNothing(m_ordet_cantidadentregada) Then
                If Not m_ordet_cantidadentregada.Equals(value) Then
                    ...
                    OnORDEN_CodigoChanged(m_ordet_cantidadentregada, EventArgs.Empty)
                End If
            Else
                m_ordet_cantidadentregada = value
                OnORDEN_CodigoChanged(m_ordet_cantidadentregada, EventArgs.Empty)
    ...
    Public Property ORDET_UsuarioAlmacen() As String
        ...
        End Get
        Set(ByVal value As String)
            If Not IsNothing(m_ordet_usuarioalmacen) Then
                If Not m_ordet_usuarioalmacen.Equals(value) Then
                    m_ordet_usuarioalmacen = value
                    OnORDEN_CodigoChanged(m_ordet_usuarioalmacen, EventArgs.Empty)
                End If
            Else
                m_ordet_usuarioalmacen = value
                OnORDEN_CodigoChanged(m_ordet_usuarioalmacen, EventArgs.Empty)
        ...
        Set(ByVal value As String)
            If Not IsNothing(m_ordet_motivo) Then
                If Not m_ordet_motivo.Equals(value) Then
                    ...
                    OnORDEN_CodigoChanged(m_ordet_motivo, EventArgs.Empty)
                End If
            Else
                m_ordet_motivo = value
                OnORDEN_CodigoChanged(m_ordet_motivo, EventArgs.Empty)
...
#Region " Eventos "
    ...
    Public Event ORDEN_ArticuloDescripcionChanged As EventHandler
    ...
    Public Event DiferenciaChanged As EventHandler
    Public Event ORDEN_Codigo_DestinoChanged As EventHandler

    Public Event ORDET_UsrCreaChanged As EventHandler
    Public Event ORDET_FecCreaChanged As EventHandler
    Public Event ORDET_UsrModChanged As EventHandler
    Public Event ORDET_FecModChanged As EventHandler

    Public Event ORDET_FaltanteChanged As EventHandler
    Public Event ORDET_StockDestinoChanged As EventHandler
    Public Event ORDET_FecAlmacenChanged As EventHandler

    Public Event ORDET_CantidadEntregadaChanged As EventHandler
    Public Event ORDET_UsuarioAlmacenChanged As EventHandler
    Public Event ORDET_MotivoChanged As EventHandler
    Private Event ORDET_CodOrigenChanged As EventHandler
    'm_orden_codorigen
    ...
    Public Sub OnORDEN_CodorigenChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
        RaiseEvent ORDET_CodOrigenChanged(sender, e)
    End Sub


    Public Sub OnORDEN_Codigo_DestinoChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
        RaiseEvent ORDEN_Codigo_DestinoChanged(sender, e)
    End Sub
    ...
    Public Sub OnDiferenciaChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
        RaiseEvent DiferenciaChanged(sender, e)
    End Sub

    'ORDEN_ArticuloDescripcionChanged

    Public Sub OnORDEN_ArticuloDescripcionChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
        RaiseEvent ORDEN_ArticuloDescripcionChanged(sender, e)
    End Sub
    ...
    End Sub
    '==============================================================================
    '==============FUNCIONES ORDENES  DETALLES 

    Public Sub OnORDET_FecCreaChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
        RaiseEvent ORDET_FecCreaChanged(sender, e)
    End Sub
    Public Sub OnORDET_UsrModChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
        RaiseEvent ORDET_UsrModChanged(sender, e)
    End Sub
    Public Sub OnORDET_FecModChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
        RaiseEvent ORDET_FecModChanged(sender, e)
    End Sub
    ...
    End Sub
    Public Sub OnORDET_FecAlmacenChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
        RaiseEvent ORDET_FecAlmacenChanged(sender, e)
    End Sub
    ...
    End Sub
    '===============================================================================================
    Public Sub OnORDET_UsuarioAlmacenChanged(ByVal sender As Object, ByVal e As EventArgs)
        ActualizarInstancia()
        RaiseEvent ORDET_UsuarioAlmacenChanged(sender, e)
    End Sub
